...
SERVER_IN="/tmp/vcf_stats_max_in_$$"
SERVER_OUT="/tmp/vcf_stats_max_out_$$"
failures=0
...
if echo "$stats_default" | jq -e '.id == 2 and .result.content[0].text' >/dev/null 2>&1; then
    echo -e "${GREEN}✓ Default get_statistics works${NC}"
else
    echo -e "${RED}✗ Default get_statistics failed${NC}"
    failures=$((failures + 1))
fi

if echo "$stats_limited" | jq -e '.id == 3 and .result.content[0].text' >/dev/null 2>&1; then
    echo -e "${GREEN}✓ get_statistics with max_chromosomes works${NC}"
else
    echo -e "${RED}✗ get_statistics with max_chromosomes failed${NC}"
    failures=$((failures + 1))
fi
